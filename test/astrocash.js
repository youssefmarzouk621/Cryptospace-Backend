...
contract("AstroCash", accounts => {
  it("should put 10000 AstroCash in the first account", async () => {
    const instance = await AstroCash.deployed();
  ...
  it("should call a function that depends on a linked library", async () => {
    const instance = await AstroCash.deployed();
    const AstroCashBalance = await instance.getBalance.call(accounts[0]);
    const AstroCashBalanceInEth = await instance.getBalanceInEth.call(
      accounts[0],
    );

    const expected = 2 * AstroCashBalance.toNumber();

    assert.equal(
      AstroCashBalanceInEth.toNumber(),
      expected,
