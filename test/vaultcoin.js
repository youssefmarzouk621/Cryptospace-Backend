...
contract("VaultCoin", accounts => {
  it("should put 10000 VaultCoin in the first account", async () => {
    const instance = await VaultCoin.deployed();
  ...
  it("should call a function that depends on a linked library", async () => {
    const instance = await VaultCoin.deployed();
    const VaultCoinBalance = await instance.getBalance.call(accounts[0]);
    const VaultCoinBalanceInEth = await instance.getBalanceInEth.call(
      accounts[0],
    );

    const expected = 2 * VaultCoinBalance.toNumber();

    assert.equal(
      VaultCoinBalanceInEth.toNumber(),
      expected,
